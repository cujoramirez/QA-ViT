...
class ConvNeXtBlock(nn.Module):
    """
    ConvNeXt-Tiny style block with Layer Scale
    """
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)
        self.norm = nn.LayerNorm(dim, eps=1e-6)
        self.pwconv1 = nn.Linear(dim, 4 * dim)  # Expand to 4x
        self.act = nn.GELU()
        self.pwconv2 = nn.Linear(4 * dim, dim)  # Project back
        
        # Layer Scale (ConvNeXt innovation)
...
class CNNStemModel(nn.Module):
    """
    ConvNeXt-Tiny style CNN backbone
    Distribution: [3, 3, 9, 3] but adapted for CIFAR (32×32 -> 8×8)
    """
    def __init__(self, in_ch=3, c2=64, c3=128, c4=256, norm_layer=nn.LayerNorm):
        super().__init__()
        
        # Patchify stem (4×4 non-overlapping, stride=4)
        # 32×32 -> 8×8 directly
        self.stem = nn.Sequential(
            nn.Conv2d(in_ch, c2, kernel_size=4, stride=4),
            nn.LayerNorm([c2, 8, 8], eps=1e-6)  # Spatial LayerNorm
        )
        
        # Stage 2: Operate at 8×8, output c2 channels
        # Using 2 blocks (reduced from 3 for efficiency)
        self.stage2 = nn.Sequential(
            ConvNeXtBlock(c2, drop_path=0.0, layer_scale_init_value=1e-6),
            ConvNeXtBlock(c2, drop_path=0.0, layer_scale_init_value=1e-6),
        )
        
        # Downsample stage2 -> stage3 (channel increase)
        self.downsample2 = nn.Sequential(
            nn.LayerNorm([c2, 8, 8], eps=1e-6),
            nn.Conv2d(c2, c3, kernel_size=1),
        )
        
        # Stage 3: Operate at 8×8, output c3 channels
        # Using 3 blocks (reduced from 9 for efficiency)
        self.stage3 = nn.Sequential(
            ConvNeXtBlock(c3, drop_path=0.0, layer_scale_init_value=1e-6),
            ConvNeXtBlock(c3, drop_path=0.1, layer_scale_init_value=1e-6),
            ConvNeXtBlock(c3, drop_path=0.1, layer_scale_init_value=1e-6),
        )
        
        # Downsample stage3 -> stage4 (channel increase)
        self.downsample3 = nn.Sequential(
            nn.LayerNorm([c3, 8, 8], eps=1e-6),
            nn.Conv2d(c3, c4, kernel_size=1),
        )
        
        # Stage 4: Operate at 8×8, output c4 channels
        # Using 2 blocks (reduced from 3 for efficiency)
        self.stage4 = nn.Sequential(
            ConvNeXtBlock(c4, drop_path=0.1, layer_scale_init_value=1e-6),
            ConvNeXtBlock(c4, drop_path=0.1, layer_scale_init_value=1e-6),
        )
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            F4: [B, c4, 8, 8]
        """
        x = self.stem(x)           # [B, c2, 8, 8]
        
        F2 = self.stage2(x)        # [B, c2, 8, 8]
        
        x = self.downsample2(F2)   # [B, c3, 8, 8]
        F3 = self.stage3(x)        # [B, c3, 8, 8]
        
        x = self.downsample3(F3)   # [B, c4, 8, 8]
        F4 = self.stage4(x)        # [B, c4, 8, 8]
