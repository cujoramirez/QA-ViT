...
class ArchitectureAnalyzer:
    ...
    def _print_memory_analysis(self):
        ...
        # Estimates
        components_mem = {
            'Input Image (B×3×224×224)': (batch_size * 3 * 224 * 224 * 4 / 1024**2, 0),
...
def compile_and_test_model(model, config, device):
    ...
    if config.use_compile and pytorch_version >= (2, 0):
        ...
        try:
            ...
            warmup_input = torch.randn(8, 3, 224, 224, device=device)
            with torch.no_grad():
                for _ in range(3):
...
def train_epoch(model, loader, optimizer, scheduler, scaler, config, epoch, monitor):
    """Training loop with detailed gradient monitoring"""
    model.train()
    total_loss = 0
    correct = 0
    total = 0
